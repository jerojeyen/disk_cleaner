...
INT main_process()
{
	...
	addToLog("--- DiskCleaner starting now ---");
	...
	addToLog("--- DiskCleaner finishing now ---");
	return (EXIT_SUCCESS);
}
...
INT clean_path(LPTSTR path)
{
	...
	concatBeforeAddToLog("Done. Number of files cleaned : ", itoa(num_done, num_done_str, 10));
	if (num_failed > 0)
		concatBeforeAddToLog("But number of files cleaning failed : ", itoa(num_failed, num_failed_str, 10));
	return (EXIT_SUCCESS);
}
...
INT read_directory(LPTSTR path, INT *num_done, INT *num_failed, INT wide_mode)
{
	...
	if ((hFind = FindFirstFile(complete_path, &FindFileData)) == INVALID_HANDLE_VALUE) {
		concatBeforeAddToLog("Error reading folder : ", get_error_msg(GetLastError()));
		return (EXIT_FAILURE);
	}
	do {
		if (strcmp((LPCTSTR)FindFileData.cFileName, "..") != 0 && strcmp((LPCTSTR)FindFileData.cFileName, ".") != 0) {
			...
			if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if (!strcmp(strDownCase(FindFileData.cFileName), "temp") || !strcmp(strDownCase(FindFileData.cFileName), "tmp")) {
					concatBeforeAddToLog("Recursively entering in (Temp folder) : ", complete_path_in);
					read_directory(complete_path_in, num_done, num_failed, 1); //Work recursively :)
				}
				else {
					concatBeforeAddToLog("Recursively entering in : ", complete_path_in);
				...
				if (DeleteFile(complete_path_in)) {
					*num_done += 1;
					addToLogHasBeenDeleted(complete_path_in);
				}
				else {
					*num_failed += 1;
					addToLogNotBeenDeleted(complete_path_in, GetLastError());
				}
			}
	...
	if (!FindClose(hFind)) {
		concatBeforeAddToLog("Error closing folder : ", get_error_msg(GetLastError()));
		return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
